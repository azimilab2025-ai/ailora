...
def test_oya_section_is_visual_sourced_and_truthful() -> None:
    ...
    assert ("Oya" in source) and (
        ...
        or ("DISABLED" in source)
        or ("disabled" in source.lower())
        or ("pending" in source.lower())
        or ("NOT CURRENTLY IMPLEMENTED" in source)
        or ("library-agent" in source.lower())
        or ("Active Qualification" in source)
    )
...
def test_target_sections_have_no_legacy_markdown_tables() -> None:
    ...
    roadmap = section(source, "## Roadmap")
    oya_start = (
        source.index("Future Integration Roadmap: Oya Voice AI")
        if "Future Integration Roadmap: Oya Voice AI" in source
        else source.index("Oya")
        if "Oya" in source
        else 0
    )
    oya_end = source.find("\n## ", oya_start)
