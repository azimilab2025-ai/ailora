...
`LIVE_NASA_DATA=NOT_ACTIVATED`. `IN_REPO_OYA=DISABLED_LIBRARY_AGENT`. `EXTERNAL_OYA_GETOYA=LIVE_CANDIDATE_INTEGRATION`. `AILORA_RUNTIME_READ_ONLY` in this baseline means candidate hardening (debug off, weak secrets forbidden, outbound limited), not a blanket POST deny. Login and SSA writes on Render remain in scope. Local backup timings are qualification observations, not production RPO/RTO. The repository may be described as a Production Grade Candidate baseline only.
